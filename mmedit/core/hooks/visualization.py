...
@HOOKS.register_module()
class VisualizationHookWithYUV(Hook):
    """Visualization hook.
    ...
    def after_train_iter(self, runner):
        ...
        newimgs = []
        img_cat = img_cat.numpy()
        for i in range(np.shape(img_cat)[0]):
            newimgs.append(yuv2rgb(np.ndarray(img_cat[i]),channel_axis=1))

        img_cat = torch.from_numpy(newimgs)
        img_cat = img_cat.clamp_(0, 1)
        save_image(
            img_cat,
            osp.join(self.output_dir, filename),
